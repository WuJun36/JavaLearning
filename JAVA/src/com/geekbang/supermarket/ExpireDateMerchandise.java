...
// >>TODO 接口无法被实例化，也就是不可以new一个接口的实例
public interface ExpireDateMerchandise {

    // >>TODO 接口里的方法都是用public abstract修饰的，方法有名字，参数和返回值，没有方法体，以分号；结束
    // TODO 接口注释最好写一下

    // >>TODO 缺省的实现方法，用default修饰，可以有方法体
    default boolean noExpireInDays(int days){
        return days > 0;
//        return daysBeforeExpire() > days;
     ...
     */
    boolean notExpireInDate(int days);
    ...
    // >>TODO 因为接口里的方法都是且只能用public abstract修饰，所以这两个修饰符可以省略
...
//    }
    ...
    // >>TODO 接口里不能定义局部变量，定义的变量都是public static final,这三个修饰符也可以省略
